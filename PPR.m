function PPR(SeqFile,ResultFile,t)
if nargin==2
    t='0';
end
...
if ~isempty(Group_A)
    ...
    cmd=['python ',pwd,'/predict.py ',pwd,'/model_a.h5'];unix(cmd);
...
if ~isempty(Group_B)
    ...
    cmd=['python ',pwd,'/predict.py ',pwd,'/model_b.h5'];unix(cmd);
    delete([pwd,'/tmp/nt_length.csv']);
    delete([pwd,'/tmp/condon_length.csv']);
    delete([pwd,'/tmp/nt_onehot.mat']);
    delete([pwd,'/tmp/condon_onehot.mat']);
...
if ~isempty(Group_C)
    ...
    cmd=['python ',pwd,'/predict.py ',pwd,'/model_c.h5'];unix(cmd);
    delete([pwd,'/tmp/nt_length.csv']);
    delete([pwd,'/tmp/condon_length.csv']);
    delete([pwd,'/tmp/nt_onehot.mat']);
    delete([pwd,'/tmp/condon_onehot.mat']);
